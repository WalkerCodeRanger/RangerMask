describe("RangerMask.backspace", function ()
{
	...
	it("should delete across fixed places", function ()
	{
		data = dataFor(mask, "945-^67-5689");
		mask.backspace(data);
		expect(maskedValueOf(mask, data)).toEqual("94^6-75-6_89");
	});
	...
	it("should delete across optional places", function ()
	{
		...
		mask.backspace(data);
		mask.backspace(data);
		expect(maskedValueOf(mask, data)).toEqual("___-__-4^_8_");
	});
	...
	it("should delete last place", function ()
	{
		...
		mask.backspace(data);
		expect(maskedValueOf(mask, data)).toEqual("___-__-456^_");
	});
	...
	it("should move back a place through empty required fields", function ()
	{
		data = dataFor(mask, "___-__^-4568");
		mask.backspace(data);
		expect(maskedValueOf(mask, data)).toEqual("___-_^4-5_68");
	});
	...
	it("should not pull when placeholders between cursor and chars", function ()
	{
		...
		mask.backspace(data);
		expect(maskedValueOf(mask, data)).toEqual("___-^__-4568");
	});
	...
	it("should not pull when placeholders between chars", function ()
	{
		...
		mask.backspace(data);
		expect(maskedValueOf(mask, data)).toEqual("^456-__-4___");
	});
});